...
class DB {
  ...
  getRoom(name) {
    const room = db.get("rooms").filter({
      name
    }).value();
  ...
  getPlan(id) {
    const plan = db.get("plans").filter({
      id
    }).value();
  ...
  addPlan(room, name, active = false) {
    ...
    if (this.getPlan(`${room}.${name}`).length > 0) throw Error("Plan already exists");
    let plan = {
      room,
      name,
      id: `${room}.${name}`,
  ...
  deletePlan(room, name) {
    if (this.getRoom(room).length === 0) throw Error("Room doesn't exists");
    if (this.getPlan(`${room}.${name}`).length === 0) throw Error("Plan doesnt't exists");
    return db.get('plans').remove({
      room,
      name
    }).write();
  }
  ...
  activatePlan(room, name) {
    if (this.getRoom(room).length === 0) throw Error("Room doesn't exists");
    if (this.getPlan(`${room}.${name}`).length === 0) throw Error("Plan doesn't exists");
  ...
  getPlans(room) {
    if (this.getRoom(room).length === 0) throw Error("Room doesn't exists");
    const plans = db.get("plans").filter({
      room
    }).value();
    return plans;
  }
  ...
  addTemp(room, plan, day, time, temp) {
    if (this.getRoom(room).length === 0) throw Error("Room doesn't exists");
    if (this.getPlan(`${room}.${plan}`).length === 0) throw Error("Plan doesn't exists");
    if (this.getTemp(room, plan, day, time).length > 0) {
      // perform update
      return db.get('temps').find({
          room,
          plan,
          day,
          time
  ...
  deleteTemp(room, plan, day, time) {
    if (this.getRoom(room).length === 0) throw Error("Room doesn't exists");
    if (this.getPlan(`${room}.${plan}`).length === 0) throw Error("Plan doesn't exists");
    if (this.getTemp(room, plan, day, time).length === 0) throw Error("Temperature doesn't exists");
    return db.get('temps').remove({
      room,
      plan,
      day,
      time
  ...
  getTemps(room, plan) {
    if (this.getRoom(room).length === 0) throw Error("Room doesn't exists");
    if (this.getPlan(`${room}.${plan}`).length === 0) throw Error("Plan doesn't exists");
    const temps = db.get("temps").filter({
      room,
      plan
    }).value();
    return temps;
  ...
  getTemp(room, plan, day, time) {
    if (this.getRoom(room).length === 0) throw Error("Room doesn't exists");
    if (this.getPlan(`${room}.${plan}`).length === 0) throw Error("Plan doesn't exists");
    const temp = db.get("temps").filter({
      room,
      plan,
      day,
      time
  ...
  getCurrentTemperature(room) {
    if (this.getRoom(room).length === 0) throw Error("Room doesn't exists");
    const activePlan = db.get("plans").filter({
      room,
      active: true
    }).head().value();
